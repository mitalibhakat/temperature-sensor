...
async def test_sipo_with_latch(dut):
    ...
    #temp_value = 0b0011011000011111  # Example temp value (36°C, binary format)
    ...
    #Assert SIPO output against expected value
    expected_output = temp_value
    #check if SIPO output matches the expected temperature value
    assert sipo_output == temp_value, f"Test failed! SIPO output {sipo_output} does not match expected temperature value {temp_value}"
    ...
    #assert latch_output == sipo_output, f"Test failed! Latch_Q = {bin(latch_output)}, expected = {bin(sipo_output)}"
